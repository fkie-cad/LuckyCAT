...
def show_crash(crash_id):
    ...
    if crash_id:
        ...
        if crash:
            ...
            flash(f'No crash with crash id {crash_id}')
            return redirect('crashes/show')
    else:
...
def search_crash():
    ...
    return render_template('crashes_search.html',
                           database_structure=crash_database_structure,
                           job_names=job_names,
                           error=error)
...
def process_search_query():
    ...
    return list(Crash.objects.aggregate(*[{'$match': query}]))
...
def download_crash(crash_id):
    ...
    if crash:
        ...
        flash(f'Unknown crash ID: {crash_id}')
        return redirect('crashes/show')
